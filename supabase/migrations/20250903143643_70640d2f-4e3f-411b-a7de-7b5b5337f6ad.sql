-- Primero, vamos a corregir la reserva problemática para que use la combinación correcta (mesas 3 y 4)
UPDATE public.reservation_table_assignments 
SET table_id = (SELECT id FROM tables WHERE name = '3') 
WHERE reservation_id = '4cc028a5-c25a-4e55-b2fc-9e7bae9f45ec' 
  AND table_id = (SELECT id FROM tables WHERE name = '2');

-- Ahora vamos a crear una función mejorada que respete las combinaciones de mesas
...
BEGIN
  ...
  IF array_length(v_assigned_tables, 1) = 0 OR v_assigned_tables IS NULL THEN
    ...
    ELSE
      -- Greedy: biggest effective capacity first (as fallback)
