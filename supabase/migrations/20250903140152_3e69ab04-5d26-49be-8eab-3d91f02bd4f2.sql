-- Fix move_reservation_with_validation to properly handle overlaps when changing duration
-- The issue is that when changing duration, the function is not correctly checking if the extended time conflicts with other reservations
...
BEGIN
  ...
  IF p_new_table_ids IS NOT NULL THEN
    -- Use specific tables provided
    v_assigned_tables := p_new_table_ids;

    -- Check capacity of selected tables
  ...
  ELSE
    ...
    -- First try: Use current tables if they're still available for the new time period
    IF v_current_table_ids IS NOT NULL AND array_length(v_current_table_ids, 1) > 0 THEN
      -- Check if current tables are available for the new time period
      IF NOT EXISTS (
        SELECT 1
        FROM public.reservation_table_assignments rta
        JOIN public.reservations r ON rta.reservation_id = r.id
        WHERE rta.table_id = ANY(v_current_table_ids)
          AND r.id != p_reservation_id
          AND r.date = p_new_date
          AND r.status != 'cancelled'
          AND (
        ...
        -- Current tables are available, check if they have enough capacity
        SELECT COALESCE(SUM(t.capacity + COALESCE(t.extra_capacity, 0)), 0) INTO v_current_capacity
        FROM public.tables t
        WHERE t.id = ANY(v_current_table_ids) AND t.is_active = true;
        
        IF v_current_capacity >= v_needed_capacity THEN
          v_assigned_tables := v_current_table_ids;
        END IF;
      END IF;
    END IF;

    -- If current tables don't work, find new ones
