-- Fix timezone consistency and overlap checks to prevent overbooking
-- All functions now compute v_start_at/v_end_at in Europe/Madrid and compare overlaps using date+time+duration to avoid legacy start_at timezone inconsistencies.
...
-- 2) create_reservation_with_assignment
CREATE OR REPLACE FUNCTION public.create_reservation_with_assignment(p_customer_id uuid, p_date date, p_time time without time zone, p_guests integer, p_special_requests text DEFAULT NULL::text, p_duration_minutes integer DEFAULT 90)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  ELSE
    -- Greedy: biggest effective capacity first
...
-- 3) create_reservation_with_specific_tables
CREATE OR REPLACE FUNCTION public.create_reservation_with_specific_tables(p_customer_id uuid, p_date date, p_time time without time zone, p_guests integer, p_special_requests text DEFAULT NULL::text, p_table_ids uuid[] DEFAULT NULL::uuid[], p_duration_minutes integer DEFAULT 90)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- 4) move_reservation_with_validation
CREATE OR REPLACE FUNCTION public.move_reservation_with_validation(p_reservation_id uuid, p_new_date date, p_new_time time without time zone, p_duration_minutes integer DEFAULT 90, p_new_table_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- 5) update_reservation_guests_with_reassignment
CREATE OR REPLACE FUNCTION public.update_reservation_guests_with_reassignment(p_reservation_id uuid, p_new_guests integer)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
