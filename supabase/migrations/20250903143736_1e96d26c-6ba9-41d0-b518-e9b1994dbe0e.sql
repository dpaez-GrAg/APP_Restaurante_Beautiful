-- Actualizar la función de movimiento de reservas para que también respete las combinaciones
...
BEGIN
  ...
  IF p_new_table_ids IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- Auto-assign tables using combination logic
    ...
    -- If no combination worked, try current tables if available
    ...
    -- If still no assignment, try individual tables
    IF array_length(v_assigned_tables, 1) = 0 OR v_assigned_tables IS NULL THEN
      v_current_capacity := 0;
